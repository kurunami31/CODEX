...
try {
  // Add AM/PM columns to attendance table
  await client.query(`
  ...
  `);
  console.log('Added AM/PM columns');

  // Backfill: move existing time_in -> time_in_am, time_out -> time_out_am
  await client.query(`
    update public.attendance
    set time_in_am = time_in, time_out_am = time_out
    where time_in_am is null;
  `);
  console.log('Backfilled existing data to AM columns');

  // Update mark_attendance RPC for AM/PM cycles
  await client.query(`
    ...
    begin
      ...
      -- Cutoff for morning/afternoon: 12:00 PM on event day
      v_cutoff_time := (e.event_date::date + time '12:00:00') at time zone 'Asia/Manila';
    ...
    $$;
  `);
  console.log('mark_attendance RPC updated with AM/PM cycles');
} catch (err) {
