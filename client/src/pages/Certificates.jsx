...
import { CertificateIcon, DownloadIcon, CalendarIcon, CheckIcon, XIcon } from '../components/icons/Icons';
...
export default function Certificates() {
  ...
  const [open, setOpen] = useState(null); // { type: 'membership' } | { type: 'event', title, date }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (error) toast.error('Certificates error', error.message);
          ...
          <div className="modal modal--wide">
            <div className="modal-head">
              <h3><CertificateIcon width={17} height={17} style={{ verticalAlign: -3, marginRight: 6, color: 'var(--accent-2)' }} />
                {open.type === 'membership' ? 'Membership certificate' : 'Event certificate'}
              </h3>
              ...
              <div className="cert-print" ref={printRef}>
                <div className="cert-frame">
                  ...
                  <div className="cert-title">
                    {open.type === 'membership' ? 'CERTIFICATE OF MEMBERSHIP' : 'CERTIFICATE OF PARTICIPATION'}
                  </div>
                  ...
                  <div className="cert-code">cert-{open.type === 'membership' ? 'member' : 'event'}-{String(profile.student_id || user?.id || '').toLowerCase().replace(/[^a-z0-9]/gi, '')}</div>
                </div>
              </div>
